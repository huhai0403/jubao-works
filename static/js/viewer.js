...
(function (factory) {
  ...
  var checkItems;
  $(document).on('click','.data_item_list img',function(){
    checkItems = $(this).parents('.items').find('.list')
  });
  ...
  $.extend(prototype, {
    ...
    renderList: function (index) {
      ...
      //底部小图列表
      this.$list.css({
        width: outerWidth * this.length*2,
    ...
    initImage: function (callback) {
      ...
      getImageSize($image[0], $.proxy(function (naturalWidth, naturalHeight) {
        ...
        /*查看图片的大小*/
        width = min(width * 0.7, naturalWidth);
        height = min(height * 0.7, naturalHeight);
    ...
    loadImage: function (e) {
      ...
      getImageSize(image, $.proxy(function (naturalWidth, naturalHeight) {
        ...
        if (parentHeight * aspectRatio > parentWidth) {
          if (filled) {
            width = parentHeight * aspectRatio + 30;
          } else {
  ...
  var removedIndex;
  //add 去除方法
  function deleteByIndex(arr,indexToDelete){
  ...
  //add 移出
  $(document).on('click','.add_ul img',function(){
    var removedIndex = $(this).index();
    $('.viewer-list li span.on').eq(removedIndex).removeClass('on');
    $(this).remove();
    addUrlArray = deleteByIndex(addUrlArray,removedIndex);
    return removedIndex;
  });
  $.extend(prototype, {
    ...
    //add 移除按钮
    delete: function(){
      //console.log(this.$list.find('img').eq(this.index).data('originalUrl'));
      //var imgNum = this.$list.find('img').length;
      addIndex = this.index;
      var $img = this.$list.find('img');
      var addUrl = this.$list.find('img').eq(this.index).data('originalUrl');
      delArray.push(addIndex);
      $.each($.unique(delArray),function(i){
        addUrlArray.push($img.eq(delArray[i]).data('originalUrl'))
      });
      //console.log($.unique(addUrlArray));
      //console.log(removedIndex);
      /*for(var i=0;i++;i<$.unique(delArray).length){
        addUrlArray.push(this.$list.find('img').eq(delArray[i]).data('originalUrl'))
        console(i)
      }
      //console.log(addUrlArray);
      //console.log($.unique(delArray));
      //console.log($.unique(delArray).length);
      //console.log(this.$list.find('img').eq(delArray[0]).data('originalUrl'));

      //console.log($.unique(delArray));

      //console.log(addIndex)
      //console.log(this.$list.find('img').eq(this.index));*/
      $('.viewer-list li span').eq(this.index).addClass('on');
      $('.add_ul .add_content').empty();
      $.each($.unique(addUrlArray),function(i,value){
        $('.add_ul .add_content').append('<img src='+value+'>');
      });

      //$('.add_ul').append('<img src='+addUrl+' data-index='+ addIndex+'>');
      //console.log(addIndex);
    ...
    '<div class="viewer-container">' +
      ...
      '<div class="viewer-footer">' +
        ...
        '<ul class="viewer-toolbar">' +
          ...
          '<li class="cancel_img" data-action="delete"><span></span></li>' +
        '</ul>' +
  ...
  $(document).on('click','.viewer-list li span',function(){
    $(this).removeClass('on')
  });

/*同步选中移出的图片*/
  $(document).on('click','.viewer-button',function(){
    /*选中的数组*/
    var checkedThisPic = [];
    $('.viewer-list span').each(function(index){
      if($('.viewer-list span').eq(index).attr('class') == 'on'){
        checkedThisPic.push(1);
      }else {
        checkedThisPic.push(0);
      }
    });
    /*数组渲染列表勾选*/
    checkItems.find('li a').each(function(index){
      if(checkedThisPic[index] == 0){
        checkItems.find('li a').eq(index).attr('class','');
      }else if(checkedThisPic[index] == 1){
        checkItems.find('li a').eq(index).attr('class','del');
      }
    });
    /*判断是否全选*/
    if($('.viewer-list span.on').length != 0){
      checkItems.parents('.items').find('.checkbox01 span').addClass('un');
    }
  });
